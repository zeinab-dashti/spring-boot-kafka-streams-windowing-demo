...
public class SlidingWindow {
    ...
    public void buildPipeline(StreamsBuilder streamsBuilder) {
        ...
        eventStream
                .groupByKey()
                .windowedBy(SlidingWindows.ofTimeDifferenceWithNoGrace(Duration.ofSeconds(15)))
                .count(Materialized.as("sliding-store"))
                .suppress(Suppressed.untilWindowCloses(Suppressed.BufferConfig.unbounded()))
                .toStream()
                .map((key, value) -> KeyValue.pair(
                        key.key(),
                        "Key = " + key.key() + ", Sliding Window Count = " + value
                ))
                .to(KafkaConfig.WINDOW_OUTPUT_TOPIC, Produced.with(Serdes.String(), Serdes.String()));

        KafkaStreams streams = new KafkaStreams(streamsBuilder.build(), KafkaConfig.getStreamProperties("sliding-stream-app", "state/sliding-store-dir"));
